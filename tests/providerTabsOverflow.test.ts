...
describe('provider filter overflow', () => {
  ...
  test('keeps the tabs strip flexible so the toolbar controls stay fixed', () => {
    // tabs 是 tabsRow 的第一个子元素：吃掉剩余宽度，但不挤压右侧控件。
    expect(quotaStyles).toMatch(/> :first-child\s*\{\s*flex: 1 1 auto;\s*min-width: 0;/);
    expect(quotaStyles).toMatch(/\.toolbarControls\s*\{[^}]*flex: 0 0 auto;/);
    expect(quotaStyles).toMatch(/\.sort\s*\{[^}]*flex: 0 0 auto;/);
  });
});
