...
const TIMELINE_SPAN_DAYS: Record<TimelineMode, number> = {
  weekly: 14,
  session: 3,
};
 ...
 * anchor, which lets the caller consistently decide whether the lane is useful.
 */
export function buildTimelineLane(input: TimelineLaneInput): TimelineLane {
  const { name, provider, quota, maxPeriodHours } = input;
      ...
      provider === 'codex'
        ? ((quota as { rateLimitResetCredits?: ResetCreditLike[] }).rateLimitResetCredits ?? [])
            ...
            .map((credit): TimelineResetCredit | null => {
              ...
              const grantedAtMs = parseIsoToMs(credit.grantedAt);
              return {
                id: credit.id ?? '',
                grantedAtMs,
                expiresAtMs,
